    ...
    this._oauth2.get('https://discordapp.com/api/users/@me', accessToken, function (err, body, res) {
        ...
        self.checkScope('connections', accessToken, function (errx, connections) {
            ...
            self.checkScope('guilds', accessToken, function (erry, guilds) {
                if (erry) done(erry);
                console.log('passport-discord checkScope(guilds) erry:', erry)
                console.log('passport-discord checkScope(guilds) profile:', profile)
...
Strategy.prototype.checkScope = function (scope, accessToken, cb) {
    if (this._scope && this._scope.indexOf(scope) !== -1) {
        console.log('passport-discord checkScope scope', scope)
        console.log('passport-discord checkScope this._scope', this._scope)
        console.log('passport-discord checkScope this._scope.indexOf(scope)', this._scope.indexOf(scope))
        this._oauth2.get('https://discordapp.com/api/users/@me/' + scope, accessToken, function (err, body, res) {
            console.log('passport-discord checkScope err', err)
            console.log('passport-discord checkScope body', body)
